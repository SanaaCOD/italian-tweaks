...
function Get-HidusbfToolStatus {
    ...
    $dir = if ($bundle.Root) { [string]$bundle.Root } else { (Join-Path $Root 'tools\hidusbf') }
    $ok = [bool]$bundle.Ready
    $msg = if ($ok) {
        'HIDUSBF bundle OK (1khz + 4khz-8khz + NoPatch)'
    } elseif ($bundle.Missing -and $bundle.Missing.Count -gt 0) {
        'HIDUSBF incomplet: ' + ($bundle.Missing -join ', ')
    } else {
        'Outil HIDUSBF manquant (tools/hidusbf)'
    }
    return @{
        available = $ok
        path      = $dir
...
function Convert-RawController {
    ...
    return [ordered]@{
        ...
        compatible         = ($HidusbfOk -and $targetReliable)
