        ...
        foreach ($alias in $FolderAliases) {
            ...
            foreach ($dir in $dirs) {
                if ($dir.Name -ieq $alias) {
                    ...
                    if (Test-Path -LiteralPath $sys) { $hits.Add($sys) | Out-Null }
                }
            }
            if ($hits.Count -eq 0) { continue }
            $preferred = $hits | Where-Object { $_ -match '(?i)\\AMD64_AS\\' } | Select-Object -First 1
...
function Get-HidusbfPatchUsbValues {
    $port = $null
    $xhci = $null
    foreach ($keyPath in @(
        'HKLM:\SYSTEM\CurrentControlSet\Services\hidusbf\Parameters',
    ...
    return [ordered]@{ PatchUSBPort = $port; PatchUSBXHCI = $xhci }
}

function Test-UsbParentHasHidusbfFilter {
    param([string]$UsbParentDeviceId)
    if (-not $UsbParentDeviceId) { return $false }
    $usbKey = 'HKLM:\SYSTEM\CurrentControlSet\Enum\' + $UsbParentDeviceId
    if (-not (Test-Path -LiteralPath $usbKey)) { return $false }
    try {
        $lf = (Get-ItemProperty -LiteralPath $usbKey -Name LowerFilters -ErrorAction Stop).LowerFilters
        return ($null -ne $lf -and @($lf) -contains 'hidusbf')
    } catch { return $false }
}

function Get-HidusbfConfirmedStatusForParent {
    param(
    ...
    $usbParent = ($UsbParentDeviceId + '').Trim()
    if (-not $usbParent -or ($usbParent -match '(?i)&MI_\d+')) {
        return [ordered]@{
            Success = $false; ConfirmedRate = 0; Status = 'Unknown'; Badge = 'NON CONFIRME'
            Optimized = $false; Verified = $false; ResponseTimeMs = 8.0
            Message = 'Parent USB invalide'
        }
    }
    $usbKey = 'HKLM:\SYSTEM\CurrentControlSet\Enum\' + $usbParent
    if (-not (Test-Path -LiteralPath $usbKey)) {
        return [ordered]@{
            Success = $false; ConfirmedRate = 0; Status = 'Unknown'; Badge = 'NON CONFIRME'
            Optimized = $false; Verified = $false; ResponseTimeMs = 8.0
            Message = 'Parent USB introuvable'
            UsbParentDeviceId = $usbParent
        }
    }
    if (-not $Vid -or -not $DevicePid) {
        $vp = Get-VidPidFromInstanceId $usbParent
        if (-not $Vid) { $Vid = $vp.Vid }
        if (-not $DevicePid) { $DevicePid = $vp.Pid }
    }
    $cap = Get-ControllerTypeAndCapability -Vid $Vid -DevicePid $DevicePid -NameBlob $usbParent
    $filterActive = Test-UsbParentHasHidusbfFilter -UsbParentDeviceId $usbParent
    $patch = Get-HidusbfPatchUsbValues
    $xhciInt = if ($null -ne $patch.PatchUSBXHCI) { [int]$patch.PatchUSBXHCI } else { -1 }

    if (-not $filterActive) {
        return [ordered]@{
            Success = $true; ConfirmedRate = 125; ConfiguredRate = 125; Status = 'Default'
            Badge = 'DEFAULT'; Optimized = $false; Verified = $true; ResponseTimeMs = 8.0
            Message = 'Default detecte'; UsbParentDeviceId = $usbParent; Vid = $Vid; Pid = $DevicePid
            HidusbfFilterPresent = $false; PatchUSBXHCI = $patch.PatchUSBXHCI; DeviceBadge = [string]$cap.Badge
        }
    }
    if ($xhciInt -eq 3) {
        return [ordered]@{
            Success = $true; ConfirmedRate = 8000; ConfiguredRate = 8000; Status = 'Optimized8000'
            Badge = 'OPTIMISE'; Optimized = $true; Verified = $true; ResponseTimeMs = 0.125
            Message = 'Boost 8000 Hz detecte'; UsbParentDeviceId = $usbParent; Vid = $Vid; Pid = $DevicePid
            HidusbfFilterPresent = $true; PatchUSBXHCI = 3; DeviceBadge = [string]$cap.Badge
        }
    }
    if ($xhciInt -eq 1) {
        return [ordered]@{
            Success = $true; ConfirmedRate = 1000; ConfiguredRate = 1000; Status = 'Optimized1000'
            Badge = 'OPTIMISE'; Optimized = $true; Verified = $true; ResponseTimeMs = 1.0
            Message = 'Boost 1000 Hz detecte'; UsbParentDeviceId = $usbParent; Vid = $Vid; Pid = $DevicePid
            HidusbfFilterPresent = $true; PatchUSBXHCI = 1; DeviceBadge = [string]$cap.Badge
        }
    }
    return [ordered]@{
        Success = $false; ConfirmedRate = 0; Status = 'Unknown'; Badge = 'NON CONFIRME'
        Optimized = $false; Verified = $false; ResponseTimeMs = 8.0
        Message = 'Statut HIDUSBF indetermine'; UsbParentDeviceId = $usbParent; Vid = $Vid; Pid = $DevicePid
        HidusbfFilterPresent = $true; PatchUSBXHCI = $patch.PatchUSBXHCI; DeviceBadge = [string]$cap.Badge
    }
}
