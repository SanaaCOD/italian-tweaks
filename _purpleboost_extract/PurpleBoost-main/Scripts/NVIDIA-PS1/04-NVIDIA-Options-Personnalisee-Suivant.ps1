...
# Clique le vrai bouton "&SUIVANT" dans le Programme d'installation NVIDIA
...
function Invoke-NoMouseButtonClick {
    ...
    Write-Host "[NVIDIA-PS1-03] $Label - Methode 1: UIAutomation InvokePattern"
    try {
        ...
        Write-Host "[NVIDIA-PS1-03] UIA InvokePattern indisponible."
    }

    Write-Host "[NVIDIA-PS1-03] $Label - Methode 2: BM_CLICK"
    [void][Win32]::SendMessage($ButtonHwnd, [Win32]::BM_CLICK, [IntPtr]::Zero, [IntPtr]::Zero)
    Start-Sleep -Seconds 1

    Write-Host "[NVIDIA-PS1-03] $Label - Methode 3: WM_COMMAND parent"
    ...
    Write-Host "[NVIDIA-PS1-03] $Label - Methode 4: WM_LBUTTONDOWN/UP poste au HWND exact"
...
Write-Host "[NVIDIA-PS1-03] Recherche de la fenetre NVIDIA..."
...
if ($win -eq [IntPtr]::Zero) {
    Write-Host "[NVIDIA-PS1-03] ERREUR: fenetre NVIDIA introuvable."
    exit 1
}

Write-Host "[NVIDIA-PS1-03] Fenetre trouvee hwnd=$win title='$(Get-Text $win)'"
...
Write-Host "[NVIDIA-PS1-03] Controles visibles avec texte :"
...
if (-not $next) {
    Write-Host "[NVIDIA-PS1-03] ERREUR: vrai bouton SUIVANT introuvable."
    exit 2
}

Write-Host "[NVIDIA-PS1-03] VRAI bouton SUIVANT trouve : hwnd=$($next.Hwnd) text='$($next.Text)' rect=($($next.Left),$($next.Top),$($next.Right),$($next.Bottom))"

Invoke-NoMouseButtonClick -Window $win -ButtonHwnd $next.Hwnd -Label "SUIVANT"

Write-Host "[NVIDIA-PS1-03] Terminé."
exit 0