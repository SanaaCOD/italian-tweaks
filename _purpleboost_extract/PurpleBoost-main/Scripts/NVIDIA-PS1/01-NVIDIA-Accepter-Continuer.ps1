# 01-NVIDIA-Accepter-Continuer-V3-NoMouse.ps1
# Clique le vrai bouton NVIDIA "&ACCEPTER ET CONTINUER"
# Sans souris, sans SetCursorPos, sans SendInput, sans coordonnées écran.
...
function Invoke-NoMouseButtonClick {
    ...
    Write-Host "[NVIDIA-PS1-V3] Methode 1: UIAutomation InvokePattern"
    try {
        ...
        Write-Host "[NVIDIA-PS1-V3] UIA InvokePattern indisponible."
    }

    Write-Host "[NVIDIA-PS1-V3] Methode 2: BM_CLICK sur HWND exact"
    [void][Win32]::SendMessage($ButtonHwnd, [Win32]::BM_CLICK, [IntPtr]::Zero, [IntPtr]::Zero)
    Start-Sleep -Seconds 1

    Write-Host "[NVIDIA-PS1-V3] Methode 3: WM_COMMAND au parent"
    ...
    Write-Host "[NVIDIA-PS1-V3] Methode 4: WM_LBUTTONDOWN/UP poste au HWND exact, sans bouger la souris"
...
Write-Host "[NVIDIA-PS1-V3] Recherche de la fenetre NVIDIA..."
...
if ($win -eq [IntPtr]::Zero) {
    Write-Host "[NVIDIA-PS1-V3] ERREUR: fenetre NVIDIA introuvable."
    exit 1
}

Write-Host "[NVIDIA-PS1-V3] Fenetre trouvee hwnd=$win title='$(Get-Text $win)'"
...
Write-Host "[NVIDIA-PS1-V3] Controles visibles avec texte :"
...
# IMPORTANT :
# Ici on cible le vrai bouton exact.
# On ne prend PAS la phrase "Cliquez sur Accepter et continuer..."
$accept = $children | Where-Object {
    $_.Enabled -eq $true -and
    (
        $_.Norm -eq "accepter et continuer" -or
        $_.Norm -eq "accept and continue" -or
        $_.Norm -eq "agree and continue"
    )
} | Sort-Object Top -Descending | Select-Object -First 1

if (-not $accept) {
    Write-Host "[NVIDIA-PS1-V3] ERREUR: vrai bouton '&ACCEPTER ET CONTINUER' introuvable."
    Write-Host "[NVIDIA-PS1-V3] Aucun clic effectue."
    exit 2
}

Write-Host "[NVIDIA-PS1-V3] VRAI bouton trouve : hwnd=$($accept.Hwnd) text='$($accept.Text)' class=$($accept.Class) rect=($($accept.Left),$($accept.Top),$($accept.Right),$($accept.Bottom))"

Invoke-NoMouseButtonClick -Window $win -ButtonHwnd $accept.Hwnd

Start-Sleep -Seconds 1

Write-Host "[NVIDIA-PS1-V3] Terminé."
exit 0