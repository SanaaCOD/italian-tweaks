...
if (-not $Uninstall -and $MaxRateHz -gt 0 -and $script:RateHz -gt $MaxRateHz) {
    Write-OcResult -Success $false -Verified $false -Code 'RATE_NOT_SUPPORTED_BY_CONTROLLER' `
        -Message 'Cette manette ne supporte pas 8000 Hz' -ConfirmedRate $MaxRateHz -ExitCode 40
    exit 40
}
...
$script:TargetHidusbfDeviceId = [string]$compositeTarget.TargetHidusbfDeviceId
...
if ($Uninstall) {
    ...
    if ($filters -notcontains 'hidusbf') {
        Write-OcResult -Success $true -Verified $true -Code 'RESTORED' -Message 'Defaut restaure' -NeedsReconnect (-not $restartOk) -ExitCode 0
        exit 0
    }
    Write-OcResult -Success $false -Verified $false -Code 'VERIFY_FAILED' -Message 'Restauration non confirmee' -ExitCode 34
    exit 34
}
