# 03-NVIDIA-Options-Suivant.ps1
# Clique le vrai bouton "&FERMER" dans le Programme d'installation NVIDIA
# Sans souris, sans clavier, sans coordonn̩es.
...
function Invoke-NoMouseButtonClick {
    ...
    Write-Host "[NVIDIA-PS1-06] $Label - Methode 1: UIAutomation InvokePattern"
    try {
        ...
        Write-Host "[NVIDIA-PS1-06] UIA InvokePattern indisponible."
    }

    Write-Host "[NVIDIA-PS1-06] $Label - Methode 2: BM_CLICK"
    [void][Win32]::SendMessage($ButtonHwnd, [Win32]::BM_CLICK, [IntPtr]::Zero, [IntPtr]::Zero)
    Start-Sleep -Seconds 1

    Write-Host "[NVIDIA-PS1-06] $Label - Methode 3: WM_COMMAND parent"
    ...
    Write-Host "[NVIDIA-PS1-06] $Label - Methode 4: WM_LBUTTONDOWN/UP poste au HWND exact"
...
Write-Host "[NVIDIA-PS1-06] Recherche de la fenetre NVIDIA..."
...
if ($win -eq [IntPtr]::Zero) {
    Write-Host "[NVIDIA-PS1-06] ERREUR: fenetre NVIDIA introuvable."
    exit 1
}

Write-Host "[NVIDIA-PS1-06] Fenetre trouvee hwnd=$win title='$(Get-Text $win)'"
...
Write-Host "[NVIDIA-PS1-06] Controles visibles avec texte :"
...
$close = $children | Where-Object {
    $_.Enabled -eq $true -and
    (
        $_.Text -eq "&FERMER" -or
        $_.Text -eq "FERMER" -or
        $_.Text -eq "&TERMINER" -or
        $_.Text -eq "TERMINER" -or
        $_.Text -eq "&CLOSE" -or
        $_.Text -eq "CLOSE" -or
        $_.Text -eq "&FINISH" -or
...
if (-not $close) {
    Write-Host "[NVIDIA-PS1-06] ERREUR: vrai bouton FERMER introuvable."
    exit 2
}

Write-Host "[NVIDIA-PS1-06] VRAI bouton FERMER trouve : hwnd=$($close.Hwnd) text='$($close.Text)' rect=($($close.Left),$($close.Top),$($close.Right),$($close.Bottom))"

Invoke-NoMouseButtonClick -Window $win -ButtonHwnd $close.Hwnd -Label "FERMER"

Write-Host "[NVIDIA-PS1-06] Termin̩."
exit 0
